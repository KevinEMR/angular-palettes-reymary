...
export const getAllPaths = (blocks: DesignerPropertiesValues, edges: any[]) => {
  ...
  const paths = flatten(map(sources, b => extractPaths(g, b)));
  return paths;
}
/**
 * Defuelve todos los paths que salen de un node
 */
export const getPathsFromNodes = (blocks: DesignerPropertiesValues, edges: any[]) => {
  /** Convertimos los bloques y edges en un graph */
  const g = buildGraph(blocks, edges);
  /** Buscamos todos los sources */
  const all = g.nodes();
  ...
  const paths = flatten(map(checkpoints, b => extractPaths(g, b)));
  return paths;
}
/**
 ...
 */
const extractPaths = (graph, block) => {
  /**
 ...
 */
const explorePaths = (g, v, current, paths) => {
  ...
  each(children, (w) => {
    explorePaths(g, w, current, paths);
  });
